...
void TFT::Rotation ( INT8U m )
{
  rotation = m % 8;
  SPI.begin();
  sendCMD(ILI9341_MADCTL);
  // sendCMD(0x36);      // Rotation
  switch (rotation)
  {
    case 0:
...
INT8U TFT::readID(void)
{
  ...
  /*  This code commented out as display returns NULL
      if(!ToF)  // data!=ID
      {
          If the next line is commented out library does not work
  ...
  */
  // do NOT delay this delay, without it the library will not initialise properly
  delay(100);
  return ToF;
}
